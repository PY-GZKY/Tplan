...
from .constants import (
    ...
    default_worker_name,
    worker_key,
    task_key, worker_key_close_expire
...
class Worker:
    ...
    def __init__(
            ...
            *,
            worker_name: Optional[str] = default_worker_name,
        ...
        for f_ in self.functions.values():
            if isinstance(f_, CronJob):
                ...
                function_ = {
                    ...
                    "function_ms": str(ms_to_datetime(as_int(time() * 1000)))
                }
            else:
                is_timer = 0
                function_ = {
    ...
    async def _poll_iteration(self, worker_name) -> None:
        ...
        # print("jobs_complete: ",self.jobs_complete)
    ...
    async def record_health(self) -> None:
        ...
        # print("健康检查：", info)
        await self.pool.set(self.health_check_key, json.dumps(info))
