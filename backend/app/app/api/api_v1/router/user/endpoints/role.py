...
from app.api.utils.emailExt import list_to_tree, dfs_tree_to_list
...
def read_roles(db: Session = Depends(deps.get_db)) -> Any:
    ...
    # 先取出所有数据再组成树结构
    roles = db.query(models.Role).options(joinedload(models.Role.role_menu).joinedload(models.RoleMenu.menu)).order_by(
...
def update_role(*, db: Session = Depends(deps.get_db), id: str, role_in: schemas.RoleUpdate, ) -> Any:
    ...
    role.update(urole)
    # 删除原有菜单
    db.query(models.RoleMenu).filter(models.RoleMenu.role_id == id).delete()
    # 新增现有菜单
    menus_list = dfs_tree_to_list(role_in.routes)
    menus_list = [models.RoleMenu(**{"role_id": role.one().id, "menu_id": menu_id}) for menu_id in menus_list]
...
def create_role(*, db: Session = Depends(deps.get_db), role_create: schemas.RoleCreate, ) -> Any:
    ...
    menus_list = [models.RoleMenu(**{"role_id": role.id, "menu_id": menu_id}) for menu_id in menus_list]
    db.bulk_save_objects(menus_list)
