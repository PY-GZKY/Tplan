...
from app.api.utils.emailExt import list_to_tree
...
def read_users(*,
    ...
    query = db.query(models.User, models.Department).outerjoin(
        ...
        models.Department, models.Department.id == models.UserDepartment.department_id)
...
def read_user(*, db: Session = Depends(deps.get_db)) -> Any:
    ...
    postOptions = db.query(models.DictData).join(
        models.DictType, models.DictType.id == models.DictData.type_id).filter(
        models.DictType.code == "post").all()
...
def read_user(*, db: Session = Depends(deps.get_db), id: int) -> Any:
    ...
    user_post = db.query(models.UserDict).outerjoin(
        models.DictData, models.DictData.id == models.UserDict.dict_id).outerjoin(
        models.DictType, models.DictType.id == models.DictData.type_id).filter(
        models.DictType.code == "post", models.UserDict.user_id == id).all()
...
def update_user(*, db: Session = Depends(deps.get_db), user: schemas.UserUpdate) -> Any:
    ...
    db.flush()
    # department
    db.query(models.UserDepartment).filter(models.UserDepartment.user_id == user_id).delete()
    user_department = {"user_id": user_id, "department_id": deptId}
    db.add(models.UserDepartment(**user_department))
    db.flush()
    # role
    ...
    db.bulk_insert_mappings(models.UserRole, user_roles)
    db.flush()
    # dict
    db.query(models.UserDict).filter(models.UserDict.user_id == user_id).delete()
    # post
    user_post = [{"user_id": user_id, "dict_id": i} for i in postIds]
    user_dict = user_post + []
    db.bulk_insert_mappings(models.UserDict, user_dict)
    db.flush()
...
def create_file(db: Session = Depends(deps.get_db), updateSupport: bool = False, file: UploadFile = File(...)):
    def check_dict_label(label, code):
        dict = db.query(models.DictData).outerjoin(
            models.DictType, models.DictType.id == models.DictData.type_id).filter(
            models.DictData.label == label, models.DictType.code == code).one()
        return dict
    ...
    try:
        ...
        for row in ws.iter_rows(min_row=2):
            ...
            posts = db.query(models.DictData).outerjoin(
                models.DictType, models.DictType.id == models.DictData.type_id).filter(
                models.DictData.label.in_(row[7].value.strip("").split(",")), models.DictType.code == "post").all()
            ...
            if not exist_user.first():
                ...
                db.flush()
                # department
                db.query(models.UserDepartment).filter(models.UserDepartment.user_id == exist_user_id).delete()
                user_department = {"user_id": exist_user_id, "department_id": department.id}
                db.add(models.UserDepartment(**user_department))
                # post
                db.query(models.UserDict).filter(models.UserDict.user_id == exist_user_id).delete()
